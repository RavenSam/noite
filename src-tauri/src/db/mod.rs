...
use dotenv::dotenv;
use models::{Folder, NewFolder, NewNote, Note};
use std::env;
...
// creates a new note
pub fn note_create(title: &str, body: &str, folder: Option<i32>) -> String {
    ...
    let qid: i32 = diesel::select(last_insert_rowid)
...
pub fn update_note(qid: i32, new_title: &str, new_body: &str, new_words_count: &i32) -> String {
    let mut connection = establish_connection();
    use notes::dsl::{body, id, title, words_count};
...
pub fn update_note_accent(qid: i32, new_accent_color: &str) -> String {
    let mut connection = establish_connection();
    use notes::dsl::{accent_color, id};

    let updated = diesel::update(notes::dsl::notes.filter(id.eq(&qid)))
        .set((accent_color.eq(new_accent_color),))
        .execute(&mut connection)
...
// *********************************************************************************
// creates a new note
pub fn create_folder(title: &str) -> String {
    let mut connection = establish_connection();
...
// Udate foolder -- title
pub fn update_folder(qid: i32, new_title: &str) -> String {
    let mut connection = establish_connection();
